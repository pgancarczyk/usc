...
const fetchActivities = async (date: Date) => {
  ...
  const activities: ActivityInsert[] = activityNodes.flatMap((node) => {
    try {
      ...
      const venueId = node
        .querySelector(".smm-studio-link")
        ?.getAttribute("href")
        ?.split("/")
        .pop();
      if (!venueId) throw "venue id is crucial";

      const name = node.querySelector(".smm-class-link.title")?.rawText.trim();

      const duration = node
        .querySelector(".smm-class-snippet__class-time")
        ?.rawText.trim()
        .split("&nbsp;&mdash;");
      if (!duration) throw "skip classes with no time specified";
